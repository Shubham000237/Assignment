...
export const modeToggle = (setState) => {
...
export const handlePercentage = (calculatorState, setCalculatorState) => {
    ...
    if (!config.Regex.operatorCheck.test(dataStr)) {
        ...
        setCalculatorState((previousState) => ({
            ...previousState,
            data: `${String(firstValue.toFixed(3))}`,
...
export const result = (calculatorState, setCalculatorState) => {
    try {
        ...
        // 9+= error
        if(!expression && config.Regex.endWithOperator.test(expression)){
        ...
        //agar zero se multiply ho to zero hi return ho
...
export const StopwatchIconButtonColors = (status, style)=>{
    if (status) {
        style = {
            iconSize: "clamp(30px, 8vw, 57px)",
            width: '40px',
            iconColor: 'black',
            cursor: 'pointer'
        };
